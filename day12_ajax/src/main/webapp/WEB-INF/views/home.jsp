...
<p>
	<a href="${cpath }/list"><button>list - Member5 목록</button></a>
</p>
...
<script>
	...
	btn02.onclick = function() {		// fetch를 호출할 때 opt를 생략하면 GET으로 처리한다
	...
	btn03.onclick = function() {
		...
		fetch(url, opt)
		.then(resp => resp.text())
		.then(text => result03.innerText = text)
	}
</script>
	...
	btn04.onclick = function() {
		...
		.then(json => {
			...
			json.forEach(dto => {
				...
				name.innerText = '이름 : ' + dto.name + ', '
				
				const age = document.createElement('span')
				age.classList.add('age')
				age.innerText = '나이 : ' + dto.age + '살'
