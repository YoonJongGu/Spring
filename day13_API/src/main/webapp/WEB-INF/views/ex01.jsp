...
<script>
	...
	const url = 'https://apis.data.go.kr/6260000/AttractionService/getAttractionKr?serviceKey=KduDR9%2Fc1e%2BIhCQnNdf2gfUV%2BDkEleHLJ93mk7MCtTzv5qwMFJLvWfKDNaMzCJWD6ZyRvMLS4%2FlU2vYx2slpaw%3D%3D&pageNo=1&numOfRows=10&resultType=json'
	fetch(url)
	.then(resp => resp.json())
	.then(json => {
// 		console.log(json)
...
// 		console.log(arr)
		...
		arr.forEach(dto => {
			...
			for(let key in dto) {	// 대상이 js객체일 때, 객체의 모든 속성이름에 접근한다
// 				console.log(key, dto[key])
			...
			// 자바스크립트에서 css 속성에 접근할 때, "-"로 연결되는 이름은 camelCase로 바꿔서 접근해야 한다
