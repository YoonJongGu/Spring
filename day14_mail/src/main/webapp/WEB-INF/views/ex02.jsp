...
// 		event.preventDefault()
// 		const email = document.querySelector('input[name="email"]').value
// 		const authNumber = document.querySelector('input[name="authNumber"]').value
// 		const url = '${cpath}/checkAuthNumber'
		
// 		fetch(url + '?email=' + email + '&authNumber=' + authNumber)
// 		.then(resp => resp.text())
// 		.then(text => {
// 			if(text == 1) {
// 				alert('인증번호가 확인되었습니다')
...
// 		}
		...
		const opt = {
			method: 'POST',
			body: JSON.stringify(ob),	// JSON 형식 문자열로 변환하여 전달
			headers: {
